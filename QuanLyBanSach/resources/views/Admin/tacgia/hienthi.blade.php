    ...
    <div class="col-11 col-s-12 content">
        <div style="margin-left: -50px" class="col-4 col-s-12 content">
            <div class="tabcontent">
                <div class="title">
                    <i class="fas fas fa-boxes"></i> Quản lý thông tin tác giảm
                    <hr>
                </div>
                <div>
                    <button style="width:140px;" class="btn-form" type="button" class="btn btn-info btn-lg"
                        data-bs-toggle="modal" data-bs-target="#add">Thêm tác giả</button>
                    <button style="width:140px;" class="btn-form" type="button" class="btn btn-info btn-lg"
                        data-bs-toggle="modal" data-bs-target="#myModal">Sửa tác giả</button>
                    <button style="width:140px;" class="btn-form" type="button" class="btn btn-info btn-lg"
                        data-bs-toggle="modal" data-bs-target="#delete">Xoá tác giả</button>
        ...
        <div class="col-8 col-s-12 content">
            <div class="tabcontent">
                <div class="title">
                    <i class="fas fas fa-boxes"></i> Danh sách tác giả
                ...
                <div class="row">
                            ...
                            <tbody>
                                @foreach ($tacgia as $dm)
                                    <tr>
                                        ...
                                        <td>{{ $dm->sdt }}</td>
                                        <td>{{ $dm->email }}</td>
                                        <td style="width:30px"> <i data-url="{{ route('tacgia.update', $dm->tG_id) }}"​
                                                type="button" data-target="#edit" data-toggle="modal" class='fas fa-edit'
                                                style='font-size:18px'></i>
                                        </td>
                                        <td style="width:30px" ><a onclick="return confirm('Bạn có muốn xóa không?')" href="{{route('kho.delete',$dm->tG_id)}}"><i  style='font-size:18px; color:black' class="fas fa-trash"></i></a></td>
                                    </tr>
                                @endforeach
                    ...
                    <div class="col-12 col-s-12 padding-box">
                        <div class="box-sum">
                            ...
                            <div style="margin-left:0px; margin-top:-10px" class="box-right">
                                {{$tacgia->appends(request()->all())->links()}}
                    ...
                    @include('Admin.tacgia.edit')
                    @include('Admin.tacgia.add')
                    @include('Admin.tacgia.delete')
                </div>
            </div>
        </div>
    </div>
            ...
            $('.fa-edit').click(function(e) {
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        ...
                        $('#name-edit').val(response.data.tG_name);
                        $('#id-edit').val(response.data.tG_id);
                        $('#diachi-edit').val(response.data.tG_diaChi);
